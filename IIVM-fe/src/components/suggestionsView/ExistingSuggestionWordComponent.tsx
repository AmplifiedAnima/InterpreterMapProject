...
export const ExistingWordSuggestionsComponent: React.FC<
  ExistingWordSuggestionsComponentProps
> = ({
  ...
  const handleRejectExisting = useCallback(
    async (suggestionId: number) => {
      try {
        await dispatch(rejectSuggestion({ suggestionId, suggestionType: 'vocabulary' })).unwrap();
        setToast({
  ...
  const groupedExistingSuggestions = useMemo(() => {
    return existingWordSuggestions.reduce((acc, suggestion) => {
      ...
      if (vocabularyItem) {
        ...
        console.warn(
          "No matching vocabulary item found for suggestion:",
          suggestion
        );
      }
      return acc;
    ...
    return Object.entries(groupedExistingSuggestions).map(
      ([term, { suggestions }]) => (
        <ExistingSuggestionGroup
          ...
          onReject={canApprove ? handleRejectExisting : undefined}
          isMobile={isMobile}
        />
      )
    );
